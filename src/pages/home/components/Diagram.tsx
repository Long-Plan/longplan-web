		...
		const fetchCourseDetails = async () => {
			const courseDetails = await Promise.all(
				courseDetailDisplays.map(async (courseGroup) => {
					const data = await getCourseDetailsByCourseNo(courseGroup.course_no);
					if (!data.result) {
						throw new Error(
																			...
																			.courseGroup.map((courseGroup) => {
																				...
																				return (
																					<div
																						...
																						className="inline-flex items-start justify-end rounded-[10px] border border-solid border-gray-300 w-[110px] h-[45px]"
																					>
																						<SubjectBox
																							primaryColor={
																								courseGroup.grade !== "W" &&
